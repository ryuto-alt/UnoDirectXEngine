#include "pch.h"
#include "DebugTrianglePipeline.h"
#include "DebugLinePipeline.h"

namespace UnoEngine {

void DebugTrianglePipeline::Initialize(
    ID3D12Device* device,
...
void DebugTrianglePipeline::CreateRootSignature(ID3D12Device* device) {
    ...
    ThrowIfFailed(
        ...
        "Failed to serialize debug triangle root signature"
    );
    ...
    ThrowIfFailed(
        ...
        "Failed to create debug triangle root signature"
    );
}

void DebugTrianglePipeline::CreatePipelineState(
    ID3D12Device* device,
    const Shader& vertexShader,
    const Shader& pixelShader,
    DXGI_FORMAT rtvFormat
) {
    // DebugLineVertexと同じレイアウト
    ...
    ThrowIfFailed(
        ...
        "Failed to create debug triangle pipeline state"
    );
}
