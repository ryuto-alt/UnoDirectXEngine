...
void VignettePostProcess::Initialize(GraphicsDevice* graphics) {
    ...
    Shader ps;
    ps.CompileFromFile(L"Shaders/PostProcess/VignettePS.hlsl", ShaderStage::Pixel);
...
void VignettePostProcess::CreateRootSignature(ID3D12Device* device) {
...
void VignettePostProcess::CreatePipelineState(ID3D12Device* device, const void* vsBlob, size_t vsSize,
...
void VignettePostProcess::CreateConstantBuffer(ID3D12Device* device) {
    constexpr uint32 bufferSize = sizeof(VignetteCB);
...
void VignettePostProcess::Apply(GraphicsDevice* graphics, RenderTexture* source, RenderTexture* destination) {
    if (!m_enabled) return;

    // Update constant buffer
    m_cbMapped->radius = m_params.radius;
    m_cbMapped->softness = m_params.softness;
    m_cbMapped->intensity = m_params.intensity;
    m_cbMapped->padding = 0.0f;
