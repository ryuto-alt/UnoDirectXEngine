...
void ChromaticAberrationPostProcess::Initialize(GraphicsDevice* graphics) {
    ...
    Shader ps;
    ps.CompileFromFile(L"Shaders/PostProcess/ChromaticAberrationPS.hlsl", ShaderStage::Pixel);
...
void ChromaticAberrationPostProcess::CreateRootSignature(ID3D12Device* device) {
...
void ChromaticAberrationPostProcess::CreatePipelineState(ID3D12Device* device, const void* vsBlob, size_t vsSize,
...
void ChromaticAberrationPostProcess::CreateConstantBuffer(ID3D12Device* device) {
    constexpr uint32 bufferSize = sizeof(ChromaticAberrationCB);
...
void ChromaticAberrationPostProcess::Apply(GraphicsDevice* graphics, RenderTexture* source, RenderTexture* destination) {
    if (!m_enabled) return;

    m_cbMapped->intensity = m_params.intensity;
    m_cbMapped->redOffset = m_params.redOffset;
    m_cbMapped->greenOffset = m_params.greenOffset;
    m_cbMapped->blueOffset = m_params.blueOffset;
